...
public class DriverStatisticDataAnalysis implements IOperationAnalysis{
	...
	public boolean startAnalysisOperation(AnalysisServer parentServer,InfoContainer statInfo)
	{
		...
		try {
			...
			sb.append("select count(*) as sum from ANA_DRIVER_STATISTIC_V_TABLE")
					.append(" where day='")
	...
	public void analysisDestOperation(AnalysisServer parentServer, InfoContainer statInfo)
	{
		...
		try{
			...
			sql.append(" select ");
			sql.append(" to_char(car_wanted_time,'yyyy-MM-dd')||driver_id as id,");
			...
			sql.append(" as finish_high_peak_total,");
			sql.append(" sum(case when ((to_char(car_wanted_time,'HH24mi')>='"+startHighPeak1+"' and to_char(car_wanted_time,'HH24mi')<='"+endHighPeak1+"')");
			...
			sql.append(" as finish_high_peak_book_total,");
			sql.append(" sum(case when ((to_char(car_wanted_time,'HH24mi')>='"+startHighPeak1+"' and to_char(car_wanted_time,'HH24mi')<='"+endHighPeak1+"')");
			...
			sql.append(" from (select * from taxi_order_list where car_wanted_time>=to_date('"+startTime+"','yyyy-mm-dd') and car_wanted_time<=to_date('"+endTime+"','yyyy-mm-dd hh24:mi:ss')) group by to_char(car_wanted_time,'yyyy-MM-dd'),driver_id");
			...
			while(rs.next())
			{
				...
				DriverStatisticInfo cInfo = new DriverStatisticInfo();
				cInfo.id = id;
				cInfo.driver_id=rs.getString("driver_id");
				...
				cInfo.high_peak_finish_total=rs.getInt("finish_high_peak_total");
				cInfo.high_peak_finish_book_total=rs.getInt("finish_high_peak_book_total");
	...
	public void endAnalysisOperation(AnalysisServer parentServer,InfoContainer statInfo) {
		...
		try{
			...
			StatementHandle stmt = conn
					.prepareStatement("insert into ana_driver_statistic_v_table"
							+ "  (id, driver_id, day, total, book_total, "
							+ " finish_total, finish_book_total, finish_assign_total, "
							+ " high_peak_total, high_peak_book_total, "
							+ " finish_high_peak_total, finish_high_peak_book_total, "
							+ " high_peak_finish_assign_total) "
							+ " values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			for(Iterator itr = this.resultMapping.keySet().iterator();itr.hasNext();){
				innerId = (String)itr.next();
				cInfo = (DriverStatisticInfo)this.resultMapping.get(innerId);
				stmt.setString(1, cInfo.id);
				stmt.setString(2, StrFilter.getNotNullString(cInfo.driver_id));
				stmt.setString(3, cInfo.day);
				stmt.setInt(4, cInfo.total);
				stmt.setInt(5, cInfo.book_total);
				stmt.setInt(6, cInfo.finish_total);
				stmt.setInt(7, cInfo.finish_book_total);
				stmt.setInt(8, cInfo.finish_assign_total);
				stmt.setInt(9, cInfo.high_peak_total);
				stmt.setInt(10, cInfo.high_peak_book_total);
				stmt.setInt(11, cInfo.high_peak_finish_total);
				stmt.setInt(12, cInfo.high_peak_finish_book_total);
				stmt.setInt(13, cInfo.high_peak_finish_assign_total);
	...
	private class DriverStatisticInfo
	{
		public String id;
