...
public class ActuralVehicleStopTimeAnalysis implements ITrackAnalysis {
	...
	public boolean startAnalysisTrack(AnalysisServer parentServer, InfoContainer statInfo) {
		...
		maxStopTime = Integer.parseInt(VarManageServer.getSingleInstance().getVarStringValue("max_stop_time"));
		speed = Integer.parseInt(VarManageServer.getSingleInstance().getVarStringValue("speed"));
		msgContent = VarManageServer.getSingleInstance().getVarStringValue("over_time_stop_alert");
	...
	public void analysisDestTrack(AnalysisServer parentServer, TrackInfo trackInfo) {
		...
		boolean first = true;
		// 从当前时间往前推5个小时，当有一个点的速度大于5，就不是停驶状态
		...
		if (plateNo != null) {
			...
			if (ret >= maxStopTime) {
				...
				if(vehicleMapping.containsKey(plateNo)){
					...
					acInfo.stopTime = acInfo.stopTime  + parentServer.getIntegerPara("circle_time").intValue()/60f;
				}
	...
	public void endAnalysisTrack(AnalysisServer parentServer, InfoContainer statInfo) {
		...
		try {
			...
			ActuralVehicleStopTimeBean bean = new ActuralVehicleStopTimeBean();
			if (conn != null) {
				conn.select(0);
			...
			ActuralVehicleStopTimeBean[] stopTimeBean = new ActuralVehicleStopTimeBean[vehicleMapping.size()];
			for (Iterator itr = this.vehicleMapping.keySet().iterator(); itr.hasNext();) {
				...
				ActuralVehicleStopTimeBean stopObj = new ActuralVehicleStopTimeBean();
				...
				stopObj.setStopTime(acturalVehicleInfo.stopTime);
		...
		System.out.println("Finish vehicle actural stop time data Analysis:" + this.toString() + " recordNum=" + num);
	}
	...
	private void sendMsgToMdt(String plateNo,int mdtId) {
		try {
			...
			buff.putShort(3, (short) (mdtId & 0xffff));
