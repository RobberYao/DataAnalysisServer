...
public class BusinessMonthAnasisServer extends AnalysisServer {
	...
	private void executeTask(Calendar anaDate) throws Exception{
		...
		try{
			...
			StatementHandle stmt = conn.prepareStatement("select * from ana_business_month_stat where stat_time between ? and ?");
			...
			String sql ="select taxi_company, dispatch_car_no, to_char(recode_time, 'hh24') hour, count(*) num from single_business_data_bs t where recode_time>= ? and recode_time< ? group by taxi_company, dispatch_car_no, to_char(recode_time, 'hh24')";
			...
			while(sets.next()){
				comId = sets.getInt("taxi_company");
				carNo = sets.getString("dispatch_car_no");
			...
			sql = "insert into ana_business_month_stat(id,stat_time,org_id,dest_no,hour_0,hour_1,hour_2,hour_3,hour_4,hour_5,hour_6,hour_7,hour_8,hour_9,hour_10,hour_11,hour_12,hour_13,hour_14,hour_15,hour_16,hour_17,hour_18,hour_19,hour_20,hour_21,hour_22,hour_23,stat_type,total,car_type) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,0,?,?)";
			stmt = conn.prepareStatement(sql);
			Iterator it = infoMap.keySet().iterator();
			int count = 0;
			while(it.hasNext()){
				rInfo = (RecordInfo)infoMap.get(it.next());
				stmt.setInt(1, (int)DbServer.getSingleInstance().getAvaliableId(conn, "ana_business_month_stat", "id"));
	...
	private class TimerTask extends FleetyTimerTask{
		public void run(){
			...
			System.out.println("Fire ExecTask BusinessMonthAnasisServer:"+GeneralConst.YYYY_MM_DD_HH.format(cal.getTime()));
			BusinessMonthAnasisServer.this.addExecTask(new ExecTask(cal));
		}
	}
	...
	private class ExecTask extends BasicTask{
		...
		public boolean execute() throws Exception{
			BusinessMonthAnasisServer.this.executeTask(this.anaDate);
			return true;
		}
		...
		public Object getFlag(){
			return "BusinessMonthAnasisServer";
		}
	}
}
