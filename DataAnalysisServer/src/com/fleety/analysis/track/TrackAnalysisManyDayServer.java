...
public class TrackAnalysisManyDayServer extends AnalysisServer {
	private TimerTask task = null;
	private ArrayList taskList = new ArrayList(4);
	public int preTime = 5;
	public int preDays = 5;
	private static TrackAnalysisManyDayServer instance = null;
	
	private TrackAnalysisManyDayServer (){}
	public static TrackAnalysisManyDayServer getSingleInstance(){
		if(instance == null){
			instance = new TrackAnalysisManyDayServer();
		}
		return instance;
	}
	...
	private class TimerTask extends FleetyTimerTask{
		public void run(){
			...
			//集中分析某个时间设置cal的值
			/*try {
				cal.setTime(new SimpleDateFormat("yyyy-MM-dd").parse("2017-04-25"));
			...
			for (int j = 0; j < preDays; j++) {			
				...
				System.out.println("逐天 添加任务:Fire ExecTask TrackAnalysisManyDayServer "+j+":"+GeneralConst.YYYY_MM_DD_HH.format(cal1.getTime())+" "+GeneralConst.YYYY_MM_DD_HH.format(cal.getTime()));
				TrackAnalysisManyDayServer.this.addExecTask(new ExecTask(cal1));
				cal.add(Calendar.DAY_OF_MONTH, -1);
			}
		}
	}
	...
	private void executeTask(Calendar anaDate) throws Exception{
		...
		System.out.println("Start Exec:red car anaserver:"+GeneralConst.YYYY_MM_DD_HH.format(sDate)+" "+GeneralConst.YYYY_MM_DD_HH.format(eDate)+" "+destList.size());
		
		ArrayList execList = new ArrayList(this.taskList.size());
		System.out.println("taskList:"+this.taskList.size());
		ITrackAnalysis analysis;
		for(int i=0;i<this.taskList.size();i++){
			analysis = (ITrackAnalysis)this.taskList.get(i);
       boolean res= analysis.startAnalysisTrack(this,statInfo);    
			if(res){
				execList.add(analysis);
			}
		}
		StringBuffer strBuff =  new StringBuffer(256);
		strBuff.append("Exec resddd Task Num:"+execList.size());
		for(Iterator itr = execList.iterator();itr.hasNext();){
			strBuff.append("\n"+itr.next().getClass().getName());
		}
		System.out.println("red carr:"+strBuff);
		...
		if(execList.size() > 0){
			...
			for(Iterator itr = destList.iterator();itr.hasNext();){
				...
				if((finishNum%100) == 0){
					System.out.println("AnalysisProcess:ResidenceAreaCarBusinessAnalysis "+finishNum+"/"+destList.size());
				}
			}
	...
	private class ExecTask extends BasicTask{
		...
		public boolean execute() throws Exception{
			TrackAnalysisManyDayServer.this.executeTask(this.anaDate);
			return true;
		}
		...
		public Object getFlag(){
			return "TrackAnalysisManyDayServer";
		}
	}
}
