...
public class ResidenceAreaCarBusinessAnalysis implements ITrackAnalysis {
	...
	private double querySingleBusinessDataBs(Date sTime) {
		...
		SingleBusinessDataBs singleBusinessDataBs = null;
		String sql = "select dispatch_car_no,date_up,sum from single_business_data_bs where car_no in (select b.car_no from car_area_plan_bind_info b inner join car_area_plan_info p on b.plan_id=p.id where to_date('"+GeneralConst.YYYY_MM_DD.format(sTime)+"','yyyy-MM-dd')>=p.start_time and to_date('"+GeneralConst.YYYY_MM_DD.format(sTime)+"','yyyy-MM-dd')<p.end_time+1) and date_up >= to_date('"+GeneralConst.YYYY_MM_DD.format(sTime)+"'||' 00:00:00','yyyy-MM-dd hh24:mi:ss') and date_up <= to_date('"+GeneralConst.YYYY_MM_DD.format(sTime)+"'||' 23:59:59','yyyy-MM-dd hh24:mi:ss')";
		try {
	...
	public void endAnalysisTrack(AnalysisServer parentServer,
		...
		System.out.println("End Analysis:" + this.toString());
	}
