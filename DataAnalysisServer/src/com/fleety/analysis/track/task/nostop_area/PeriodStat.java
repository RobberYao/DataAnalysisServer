...
public class PeriodStat {
	...
	StringBuffer stopDeatil = new StringBuffer(256);

	public PeriodStat() {
		super();
	}

	public void stat(DestInfo destInfo,int level2time,int level3time ) {
		...
		long totalTimes = lastTime.getTime()-firstTime.getTime();
		if(level2time>0){
			if(totalTimes>=level2time*60*1000){
				alarm_level=2;
				if(level3time>0){
					if(totalTimes>=level3time*60*1000){
						alarm_level=3;
					}
				}
			}
